...
const FOLLOWING_COL = 'following'

export async function followMama(followerId, followingUserId, mamaName) {
  // Use a composite key to prevent duplicates
  const docId = `${followerId}_${followingUserId}`
  await setDoc(doc(db, FOLLOWING_COL, docId), {
    followerId,
    followingUserId,
    mamaName: mamaName || 'A mama',
    createdAt: serverTimestamp(),
  })
}

export async function unfollowMama(followerId, followingUserId) {
  const docId = `${followerId}_${followingUserId}`
  await deleteDoc(doc(db, FOLLOWING_COL, docId))
}

export async function getFollowing(followerId) {
  const q = query(
    collection(db, FOLLOWING_COL),
...
export async function isFollowing(followerId, followingUserId) {
  const docId = `${followerId}_${followingUserId}`
  const snapshot = await getDoc(doc(db, FOLLOWING_COL, docId))
  return snapshot.exists()
}

export async function getFollowingEntries(followerId) {
  // Get all mamas this user follows
  const following = await getFollowing(followerId)
  if (following.length === 0) return []

  // Fetch shared entries for each followed mama
  const allEntries = []
  for (const f of following) {
    const q = query(
      collection(db, ENTRIES_COL),
      where('userId', '==', f.followingUserId),
      where('visibility', '==', 'shared')
    )
    const snapshot = await getDocs(q)
